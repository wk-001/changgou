...
public class SKUServiceImplv1 implements SKUService {
    ...
    public Map<String, Object> search(Map<String, String> searchMap) {
        ...
        if (searchMap == null || StringUtils.isEmpty(searchMap.get("category"))) {
            List<String> categoryList = searchCategoryList(queryBuilder);
        ...
        if (searchMap == null || StringUtils.isEmpty(searchMap.get("brand"))) {
            List<String> brandList = searchBrandList(queryBuilder);
            resultMap.put("brandList", brandList);
        }

        //规格查询
        Map<String, Set<String>> specList = searchSpecList(queryBuilder);
...
//        AggregatedPage<SKUInfo> page = elasticsearchTemplate.queryForPage(queryBuilder.build(), SKUInfo.class);
     ...
     * @return
     */
    private List<String> searchCategoryList(NativeSearchQueryBuilder queryBuilder) {
        /**
        ...
        * */
        ...
        for (StringTerms.Bucket bucket : categoryGroup.getBuckets()) {
     ...
     * @return
     */
    private List<String> searchBrandList(NativeSearchQueryBuilder queryBuilder) {
        /**
         * addAggregation()：添加一个聚合操作
         * AggregationBuilders：聚合构建对象
         * skuBrandGroup：域的别名
         * field("brandName")：根据brandName这个域进行分组查询
         */
        ...
        /*获取分组数据
        * group.getAggregations()：根据别名数据，获取的是聚合数据集合，可以根据多个域进行分组
        * get("skuBrandgroup")：获取指定域的集合数据
        * */
        List<String> brandList = new ArrayList<>();
        StringTerms brandGroup = group.getAggregations().get("skuBrandGroup");
        for (StringTerms.Bucket bucket : brandGroup.getBuckets()) {
    ...
    /**
     * 分组查询规格集合，结果：{"手机屏幕尺寸":["5寸","5.5寸"],"网络":["移动4G","联通4G","电信4G"],"颜色":["红","紫","白","蓝"]}
     * @param queryBuilder
     * @return
     */
    private Map<String, Set<String>> searchSpecList(NativeSearchQueryBuilder queryBuilder) {
        /**
         * addAggregation()：添加一个聚合操作
         * AggregationBuilders：聚合构建对象
         * skuSpecGroup：域的别名
         * field("spec.keyword  ")：根据specName这个域进行分组查询，keyword表示不分词
         */
        ...
        /*获取分组数据
        * group.getAggregations()：根据别名数据，获取的是聚合数据集合，可以根据多个域进行分组
        * get("skuSpecgroup")：获取指定域的集合数据
        * 取出的数据格式：[{"电视音响效果":"小影院","电视屏幕尺寸":"20英寸","尺码":"165"},{"电视音响效果":"小影院","电视屏幕尺寸":"20英寸","尺码":"165"}]
        * */
        List<String> specList = new ArrayList<>();
        StringTerms specGroup = group.getAggregations().get("skuSpecGroup");
        for (StringTerms.Bucket bucket : specGroup.getBuckets()) {
            String specName = bucket.getKeyAsString(); //其中一个品牌的名字
